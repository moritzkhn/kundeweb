 ...
 */
import { type Kunde, type Geschlecht, type Familienstand, type Kategorie } from './kunde';
...
export class KundeReadService {
    ...
    #suchkriterienToHttpParams(
        ...
        if (nachname !== '') {
            httpParams = httpParams.set('name', nachname);
        }
        if (kategorie !== '') {
            httpParams = httpParams.set('kategorie', kategorie);
        }
        if (geburtsdatum !== '') {
            httpParams = httpParams.set('geburtsdatum', geburtsdatum);
        }
        if (javascript) {
